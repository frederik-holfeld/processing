...
for (int i = 0; i < trees.length; i = i + 2) {
  for (int j = 1; j <= 1000; j++) {
    ...
    float trunkGrey = random(-10, 10);
    fill(121 + trunkGrey, 121 + trunkGrey, 121 + trunkGrey);
    rect(posX + size / 3, posY, size / 3, size);

    //fill(25 + random(-10, 10), 122 + random(-10, 10), 40 + random(-10, 10));
    float leafGrey = random(-10, 10);
    fill(122 + leafGrey, 122 + leafGrey, 122 + leafGrey);
